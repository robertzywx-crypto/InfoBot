...
// --- MENU DE JOGOS ACIONADO POR TEXTO ---
function mostrarMenuJogos() {
    botSay("🎮 **MENU DE JOGOS DO INFORBOT**<br><br>" +
           "Roberto, escolha uma opção digitando apenas a letra:<br><br>" +
           ...
           "**Dica:** Para sair de um jogo, digite 'parar'.");
}
...
function iniciarJogo(letra) {
    ...
    if (l === "a") {
        ...
        let n1 = Math.floor(Math.random() * 150), n2 = Math.floor(Math.random() * 150);
        respostaCerta = n1 + n2;
        ...
        const palavras = ["inforbot", "roberto", "computador", "ciencia", "anatomia"];
...
async function analyze(msg) {
    ...
    // 2. ATALHOS: SE ESCREVER 'JOGOS' OU AS LETRAS (A,B,C,D,E)
    if (raw === "jogos" || raw === "jogo") return mostrarMenuJogos();
    if (["a", "b", "c", "d", "e"].includes(raw)) return iniciarJogo(raw);
